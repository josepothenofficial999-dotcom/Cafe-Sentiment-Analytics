...
In addition to business analytics, this project includes a machine learning–based sentiment classification module.

### ML Approach
- Review text was used as input features
- Sentiment labels were derived from ratings:
  - Rating ≥ 4 → Positive
  - Rating ≤ 2 → Negative
  - Rating = 3 was excluded due to ambiguity
- Text was vectorized using TF-IDF
- Logistic Regression was used as the baseline classifier

### ML Results
- The model achieved moderate accuracy (~50%)
- Performance was limited due to:
  - Short and ambiguous review text
  - Small effective vocabulary
  - Inherent noise in text-only sentiment prediction

### Key Learning
The ML results highlight that while text-based sentiment models are useful, **rating-based sentiment remains more reliable for business decision-making**.  
Therefore, ML is used as a comparative and exploratory tool rather than the primary decision driver.